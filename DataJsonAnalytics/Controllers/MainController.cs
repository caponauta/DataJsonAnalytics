...
namespace DataJsonAnalytics.Controllers
{
    ...
    public class MainController : ControllerBase
    {
        [Route("save-data")]
        [HttpPost]
        public HttpResponseMessage SaveData(List<Conversation> lstConversation)
        {
            HttpResponseMessage httpResponseMessage = new HttpResponseMessage();

            try
            {
                foreach (var conversation in lstConversation)
                {
                        ...
                        if (conversation.participants.Count > 0)
                        {
                            foreach (var itemParticipant in conversation.participants)
                            {
                                ...
                                if (itemParticipant.sessions.Count > 0)
                                {
                                    ...
                                    {
                                        string QuerySession = "insert into sessions values ('" + itemSession.sessionId + "', '" + itemParticipant.participantId + "','" + itemSession.mediaType + "','" + itemSession.direction + "', '" + itemSession.peerId + "', '" + itemSession.provider + "', '" + (itemSession.requestedRoutings == null ? "" : itemSession.requestedRoutings[0]) + "','" + (System.String.IsNullOrEmpty(itemSession.selectedAgentId) ? "" : itemSession.selectedAgentId) + "','" + itemSession.remote + "');";
        ...
        public void SaveDataAdo(string Command)
        {
            try
            {
                ...
                //var connection = "Host=190.168.0.18:5432;Username=dbuser;Password=c0nversat1onMod3lToSql;Database=conversations_db";
